...
public void clearActivityLog() {
    ...
    private void initializeComponents() {
        ...
        wordTable.setGridColor(new Color(230, 230, 230));
        wordTable.setIntercellSpacing(new Dimension(1, 1));
        
        // Custom header renderer để đảm bảo chữ trắng trên nền xanh
        wordTable.getTableHeader().setDefaultRenderer(new javax.swing.table.DefaultTableCellRenderer() {
            @Override
        ...
        wordTable.getTableHeader().setPreferredSize(new Dimension(0, 40));
        ...
        // Custom header renderer cho activity table
        activityTable.getTableHeader().setDefaultRenderer(new javax.swing.table.DefaultTableCellRenderer() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value,
                    boolean isSelected, boolean hasFocus, int row, int column) {
                JLabel label = new JLabel(value.toString());
        ...
        activityTable.getTableHeader().setPreferredSize(new Dimension(0, 35));
    ...
    private void setupLayout() {
        ...
        headerCard.add(controlPanel, BorderLayout.CENTER);
        ...
        headerCard.add(infoPanel, BorderLayout.SOUTH);
    ...
    g.drawString(title, x, y);
        ...
        // Dictionary panel with modern card
        ...
        JLabel tableTitle = new JLabel("QUẢN LÝ TỪ ĐIỂN", SwingConstants.LEFT);
        ...
        JPanel searchPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 5));
        ...
        // Action buttons panel with better spacing
        ...
        // Activity log panel with modern card
        JPanel logPanel = createModernCard();
        logPanel.setLayout(new BorderLayout(15, 15));
        ...
        // Log header
        JPanel logHeader = new JPanel(new BorderLayout(10, 10));
        logHeader.setOpaque(false);
        JLabel logLabel = new JLabel("LỊCH SỬ HOẠT ĐỘNG", SwingConstants.LEFT);
        logLabel.setFont(new Font("Segoe UI", Font.BOLD, 22));
        logLabel.setForeground(new Color(76, 175, 80));
    ...
    // Helper method to create modern card panel
    ...
    private void displayWords(List<Word> words) {
        ...
        // Sắp xếp: từ có ảnh lên đầu, sau đó theo alphabet
        java.util.List<Word> sortedWords = new java.util.ArrayList<>(words);
        sortedWords.sort((w1, w2) -> {
            // Kiểm tra trạng thái ảnh
            boolean has1 = w1.getImagePath() != null && !w1.getImagePath().isEmpty() 
                          && new java.io.File(w1.getImagePath()).exists();
            boolean has2 = w2.getImagePath() != null && !w2.getImagePath().isEmpty() 
                          && new java.io.File(w2.getImagePath()).exists();
            
            // Từ có ảnh lên trước
            if (has1 && !has2) return -1;
            if (!has1 && has2) return 1;
            
            // Cùng trạng thái ảnh thì sắp xếp theo alphabet
            return w1.getEnglishWord().compareToIgnoreCase(w2.getEnglishWord());
        });
